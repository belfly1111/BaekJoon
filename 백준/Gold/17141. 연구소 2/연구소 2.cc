...
int main()
{
	...
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			...
			// 비어 있는 방 개수를 셈.
			if (board[i][j] != 1) emptyRoom++;

			// 바이러스가 생길 수 있는 좌표를 저장함.
			if (board[i][j] == 2)
			{
				virusPoint.push({ i,j });
				board[i][j] = 0;
	...
	// 이 때, 조합을 사용해 -> prev_permutation 바이러스 좌표 조합의 수를 구해 브루트포스를 사용한다.
...
int spreadVirus() {
	...
	virusRoomCount = virusRoomCount + M;
	...
	// 바이러스가 방 전체에 퍼지지 못했을 경우 '-1' 리턴
	if (virusRoomCount != emptyRoom)
	{
		return -1;
	}
